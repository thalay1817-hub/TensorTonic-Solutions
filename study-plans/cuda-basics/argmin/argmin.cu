...
__global__ void argmin_block_kernel(const float* input, float* block_vals, int* block_idxs, int N) {
...
__global__ void argmin_final_kernel(const float* block_vals, const int* block_idxs, int* result, int numBlocks) {
    __shared__ float sval[256];
    __shared__ int sidx[256];
...
extern "C" void solve(const float* input, float* result, int N) {
    ...
    argmin_block_kernel<<<blocksPerGrid, threadsPerBlock>>>(input, d_block_vals, d_block_idxs, N);

    int finalThreads = 256;
    argmin_final_kernel<<<1, finalThreads>>>(d_block_vals, d_block_idxs, result_int, blocksPerGrid);
