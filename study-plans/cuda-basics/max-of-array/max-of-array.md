...
$$
\text{result} = \max_{0 \le i < N} \text{input}[i]
$$

<span style="font-size: 14px;">The combining operator has an **identity** of $-\text{FLT\_MAX}$: any thread without a real element to contribute initializes its slot to that value so it can never win the comparison. The structure to exploit is the operator, not the layout - because `max` is associative, the comparisons can be reassociated into a balanced tree.</span>

---

## <span style="font-size: 16px;">Parallelization Strategy</span>

<span style="font-size: 14px;">A serial scan takes $N$ steps. The parallel form reorganizes it into a **tree reduction**: pairs of values are compared simultaneously with `fmaxf`, then pairs of partial maxima, and so on, finishing in $\log_2 N$ steps. Each block claims a contiguous chunk, reduces it to one **block partial** maximum, and the partials are then combined across blocks. This two-level structure - reduce within a block, then combine across blocks - is the defining shape of every CUDA reduction.</span>

<span style="font-size: 14px;">Within a block, each thread loads one or more elements with its global index `blockIdx.x * blockDim.x + threadIdx.x` into `__shared__` memory, then the block walks the tree. At each step the active half of the threads take `fmaxf` of their slot and one a fixed stride away, the stride halves, and a `__syncthreads()` separates the steps so every write of one level is visible before the next reads it. After $\log_2(\text{blockDim.x})$ steps, slot $0$ holds the block maximum.</span>
...
<span style="font-size: 14px;">The tree reduces one block, but the block partials still have to become one number, and partials in different blocks cannot see each other through shared memory. The combine for `max` is harder than for `sum` because CUDA has no native floating-point atomic maximum: `atomicAdd` exists for `float`, but `atomicMax` is provided only for integer types. Two strategies bridge the gap.</span>

<span style="font-size: 14px;">The first is a **CAS-based atomic-max**: build the operation out of `atomicCAS` in a loop, reading the current global value, computing `fmaxf` with the block partial, and compare-and-swapping it back, retrying if another block changed it in between. This works but adds a retry loop and, like any atomic, serializes concurrent writers to the one address. The two-level structure keeps it cheap: reduce the whole block in shared memory first, then issue one CAS-based update per block, turning $N$ contending operations into $\lceil N / \text{blockDim.x} \rceil$ of them.</span>

<span style="font-size: 14px;">The second is a **two-kernel partials approach**: the first kernel writes one partial maximum per block into a global buffer with no atomics, and a second launch reduces that small buffer. The kernel-launch boundary acts as the global barrier CUDA does not otherwise provide across blocks. For `max` this is often the cleaner choice precisely because the float atomic is not native - it sidesteps the CAS retry loop entirely, trading one extra launch for simpler, contention-free code.</span>
...
<span style="font-size: 14px;">The optimized form replaces those final levels with a **warp-shuffle reduction**: the `__shfl_down_sync` intrinsic lets a lane read another lane's register directly, feeding `fmaxf` with no shared memory and no barrier. The block reduces down to one partial per warp in shared memory, then a single warp finishes the combine in registers via shuffles. This removes the tail of `__syncthreads()` calls and the shared-memory round-trips for the last $\log_2 32 = 5$ steps, the dominant cost once the data is small. Using branchless `fmaxf` rather than an `if (a > b)` also avoids **warp divergence** on the comparison.</span>

---

## <span style="font-size: 16px;">Worked Example</span>

<span style="font-size: 14px;">Take a block of 8 threads reducing $[3, 1, 7, 0, 4, 9, 6, 2]$ in shared memory. The tree finishes in $\log_2 8 = 3$ steps, each followed by a `__syncthreads()`:</span>

* <span style="font-size: 14px;">**Step 1** (stride 4): threads $0..3$ take `fmaxf` with the slot 4 away. Slots become $[4, 9, 7, 2, \dots]$ - the upper half is now dead.</span>
* <span style="font-size: 14px;">**Step 2** (stride 2): threads $0,1$ take `fmaxf` with the slot 2 away. Slots become $[7, 9, \dots]$.</span>
* <span style="font-size: 14px;">**Step 3** (stride 1): thread $0$ takes `fmaxf` of slots $0$ and $1$. Slot $0$ holds $9$, the block maximum.</span>

<span style="font-size: 14px;">Eight values collapsed to one in three parallel steps instead of seven serial comparisons, with the active lanes halving each step. This single partial is then combined across blocks by a CAS-based atomic-max or written to the partials buffer.</span>

---

## <span style="font-size: 16px;">Pitfalls</span>

* <span style="font-size: 14px;">**Wrong identity value.** Threads with no real element must seed their slot with $-\text{FLT\_MAX}$; initializing to $0$ silently corrupts the result for all-negative inputs.</span>
* <span style="font-size: 14px;">**Assuming a native float `atomicMax`.** CUDA provides it only for integers, so the cross-block combine needs a `atomicCAS` loop or a two-kernel buffer; calling a nonexistent overload fails to compile or quietly uses the wrong type.</span>
