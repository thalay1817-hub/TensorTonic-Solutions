# <span style="font-size: 20px;">Argmax</span>

<span style="font-size: 14px;">Argmax returns the index of the largest element of an array rather than the value itself, breaking ties toward the lowest index. It is a **reduction**, but a special one: the quantity being compared (the value) is not the quantity being returned (the index). This makes it the canonical **payload reduction** - the tree does not collapse scalars, it collapses `(value, index)` pairs, comparing on the value while carrying the index along for the ride. Everything that makes a plain max reduction work still applies, but every combine step now has to move twice the data and obey a tie rule, which is where the systems interest lives.</span>
...
$$
\text{result} = \arg\max_{0 \le i < N} \text{input}[i]
$$
...
<span style="font-size: 14px;">When the grid rounds up, the tail block has threads whose `idx` runs past $N$. Those surplus lanes must not contribute a real candidate, so they seed a **sentinel pair**: a value of negative infinity and an out-of-range index. The sentinel always loses the comparison, so the guarded tail threads still participate in the tree shape without polluting the result. This is the payload-reduction equivalent of the simple `if (idx < N)` bounds check on a map - the bounds check still exists, it just resolves to a neutral element instead of an early exit.</span>
...
<span style="font-size: 14px;">2. **Atomic combine**: a single `atomicMax`-style operation will not preserve the index, so a pair reduction instead packs value and index into one 64-bit word and uses a CAS loop, or uses one atomic per block to serialize only the handful of cross-block writers. Many threads contending on one address serialize, so the block-local tree first, one atomic per block second, keeps contention to `gridDim.x` writers instead of $N$.</span>

---

## <span style="font-size: 16px;">Hardware Utilization and Latency Hiding</span>

<span style="font-size: 14px;">A global-memory load costs hundreds of cycles. The GPU hides that latency not with caches but with **massive multithreading**: when a warp issues its load of `input[idx]` and stalls, the SM scheduler switches to another resident warp. High occupancy means there is always another warp ready, so the memory pipeline stays saturated while individual loads are in flight. For the initial load phase, argmax behaves exactly like a streaming map - one coalesced read per thread - and occupancy is set simply by launching enough blocks.</span>
...
<span style="font-size: 14px;">Take 8 elements `input = [3, 9, 9, 1, 4, 9, 2, 0]` reduced in one block, comparing pairs at stride 4, then 2, then 1. Each step keeps the larger value, and on a tie keeps the lower index.</span>

* <span style="font-size: 14px;">**Step 1 (stride 4):** pairs $(0,4),(1,5),(2,6),(3,7)$. Lane 0 compares value 3 vs 4, keeps `(4, idx 4)`. Lane 1 compares 9 vs 9 - a tie, so it keeps the lower index `(9, idx 1)`. Lane 2 keeps `(9, idx 2)`, lane 3 keeps `(1, idx 3)`.</span>
* <span style="font-size: 14px;">**Step 2 (stride 2):** lane 0 compares `(4, idx 4)` vs `(9, idx 2)`, keeps `(9, idx 2)`. Lane 1 keeps `(9, idx 1)`.</span>
* <span style="font-size: 14px;">**Step 3 (stride 1):** lane 0 compares `(9, idx 2)` vs `(9, idx 1)` - a tie, keeps the lower index `(9, idx 1)`.</span>
...
* <span style="font-size: 14px;">**Reducing values and dropping the index.** A plain `fmaxf` tree finds the maximum value but loses which lane held it; the pair must travel together through every step.</span>
...
* <span style="font-size: 14px;">**Missing `__syncthreads()` between tree levels.** Reading a neighbor's pair before it is written is a race that yields wrong indices on some runs only.</span>
